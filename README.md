...
> 2. 登录 `MySQL` ，创建 `book_house` 数据库，将 `src/main/resources/book_house.sql` 文件中的数据库表导入 book_house 数据库中
> 3. 修改 `src/main/resources/jdbc.properties` 中的数据库连接信息，设置你自己的数据库用户名和密码 
> 4. 可选：修改 `src/main/resources/email.properties` 中的邮箱连接信息，设置为你自己的邮箱账号和服务器（不设置则注册时获取邮箱验证码功能不可用）。个人邮箱开启 smtp 功能指导博客：[smtp 开启](https://blog.csdn.net/smilehappiness/article/details/108145215)
> 5. 在 `IntelliJ IDEA` 中部署 `Tomcat` 即可访问
>
> 方案二：
>
> 1. 将 `RELEASE` 目录下的 `book_house.war` 包拷贝到 `Tomcat` 安装目录下的 `webapps` 目录中
>
> 2. 登录 `MySQL`，创建 `book_house` 数据库，将 `RELEASE/book_house.sql` 文件中的数据库表导入 book_house 数据库中
>
...
>
> 5. 在浏览器地址栏输入 `http://localhost:8080/book-house` 即可访问
